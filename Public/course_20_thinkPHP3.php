 ...
 *  三、ThinkPHP的视图使用
 *      1、定义
 *          thinkPHP的视图采用模板(不是Smarty模板，是thinkPHP自定义的模板)，
 *          每个模块的模板都是单独定义的，为了对模板文件进行有效的管理，thinkphp规定了
 *          默认的模板文件目录结构：
 *          视图目录/[模板主题/]控制器名/操作名+模板后缀
 *          Home/View/blue/Class/reg.html
 *          Home/View/red/Class/reg.html
 *          Home/View/orange/Class/reg.html
 ...
 *              {:函数名($变量名)}
 *              {:substr($str,0,3)}
 *              注意：若要在模板文件中调用函数，则要求此函数必须至少要有一个参数（当前变量值作为参数），
 *              且必须要有一个返回值。
 ...
 *                注意：name属性填控制器.方法中保存的二维数组名称，不加$
 *                id属性是每次循环的数组元素的变量名称，也不加$，但在标签之间使用时要加$.
 *                a、遍历所有数据
 *                  <volist name=“数组名” id=“循环变量名”> … </volist>
 *                b、遍历部分数据
 *                  <volist name=“” id=“” offset=“起始索引” length=“长度”> ... </volist>
 *                c、控制输出的记录
 *                  mod属性值为任意整数，表示以数组的索引去模以该值，在循环中可以通过{$mod}提取运算结果，从而通过该结果执行某些操作
