 ...
 *  3、ThinkPHP模块的分类
 *      3.1、公共模块(Common)
 *          概念：公共模块，也被称为应用公共模块，该模块中的配置文件(Conf/config.php)和公共函数文件(Common/Common/function.php)会在访问其它模块前被加载
 *          特点：公共模块不能直接通过URL访问 
 ...
 *          此种方法如果控制器和模型不存在将创建，存在不创建。创建的控制器继承Think\Controller，模型继承Think\Model。如果要继承自定义的控制器父类或模型类，手动创建不要在应用入口文件中写配置
 ...
 *          B、在admin.php配置文件中设置常量，基本与index.php相同，只需要添加define(“BIND_MODULE”,”Admin”),此句表示这个入口文件与Admin绑定，通过admin.php文件只能访问Admin模块
 *          C、可以将入口文件与模块的某个控制器的某个方法绑定，设置如下：define(“BIND_CONTROLLER”,“控制器名称"); 
 *          define("BIND_ACTION“,”方法名“);，但注意添加此设置表示入口文件只能访问模块的指定控制器的指定方法，其它控制器和方法均不能访问，所以一般不做此配置
 *  5、ThinkPHP的URL模式********(面试)
 *      ThinkPHP采用单一入口的设计，所有的请求都从一个入口文件开始，由入口文件提取URL中传递
 *      的模块、控制器和操作名称从而调用对应的模块，控制器和操作方法，我们把这种方式成为URL模式。
 ...
 *          例如：http://localhost/mytkp/index.php/home/index/index/name/aaa/pwd/111
 *          注意：模块、控制器、操作不区分大小写
 ...
 *        3、REWRITE模式：
 *          REWRITE模式是在PATHINFO模式的基础上添加了重写规则的支持，可以去掉URL地址里面的
 *          入口文件index.php，但是需要额外配置WEB服务器的重写规则
 *          实现步骤：
